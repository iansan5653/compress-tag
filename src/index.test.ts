...
suite("Shorthand Function Names", function() {
  context("compress", function() {
    describe("properly resolves template literals", function() {
      ...
      it("should not modify single line strings", function() {
        assert.strictEqual(`this IS a tEst`, c`this IS a tEst`);
      });

      it("should handle string placeholders properly", function() {
        assert.strictEqual(
          `this ${"is"} a ${"test with"} some ${""} strings`,
          c`this ${"is"} a ${"test with"} some ${""} strings`
        );

        assert.strictEqual(
          `this has ${""}empty ${""} strings`,
          c`this has ${""}empty ${""} strings`
        );
      });

      it("should handle numeric placeholders properly", function() {
        assert.strictEqual(`${1} | ${2} | ${0}`, c`${1} | ${2} | ${0}`);
        assert.strictEqual(`${NaN}`, c`${NaN}`, "NaN placeholder");
        assert.strictEqual(
          `${Infinity}`,
          c`${Infinity}`,
          "Infinity placeholder"
      ...
      it("should handle expression placeholders properly", function() {
        assert.strictEqual(`${1 + 456}`, c`${1 + 456}`);
        assert.strictEqual(`${Math.sqrt(2)}`, c`${Math.sqrt(2)}`);
      });

      it("should handle undefined placeholder properly", function() {
        assert.strictEqual(`${undefined}`, c`${undefined}`);
      });

      it("should handle null placeholder properly", function() {
        assert.strictEqual(`${null}`, c`${null}`);
      });

      it("should handle object placeholder properly", function() {
        const obj = {x: 56};
        assert.strictEqual(`${obj}`, c`${obj}`);
      });
    });
  ...
  a new line`,
      ...
      it("should remove CRLF newlines", function() {
        assert.strictEqual(c`this has \r\n a new line`, "this has a new line");
      });
  ...
  this has a leading new line`,
      ...
      it("should remove trailing newlines without leaving trailing spaces", function() {
        assert.strictEqual(
          c`this has a trailing new line
  `,
          "this has a trailing new line"
        );
      });
  ...
  context("compressTight", function() {
    describe("properly resolves template literals", function() {
      ...
      it("should not modify single line strings", function() {
        assert.strictEqual(`this IS a tEst`, t`this IS a tEst`);
      });

      it("should handle string placeholders properly", function() {
        assert.strictEqual(
          `this ${"is"} a ${"test with"} some ${""} strings`,
          t`this ${"is"} a ${"test with"} some ${""} strings`
        );

        assert.strictEqual(
          `this has ${""}empty ${""} strings`,
          t`this has ${""}empty ${""} strings`
        );
      });

      it("should handle numeric placeholders properly", function() {
        assert.strictEqual(`${1} | ${2} | ${0}`, t`${1} | ${2} | ${0}`);
        assert.strictEqual(`${NaN}`, t`${NaN}`, "NaN placeholder");
        assert.strictEqual(`${Infinity}`, t`${Infinity}`);
      });

      it("should handle expression placeholders properly", function() {
        assert.strictEqual(`${1 + 456}`, t`${1 + 456}`);
        assert.strictEqual(`${Math.sqrt(2)}`, t`${Math.sqrt(2)}`);
      });

      it("should handle undefined placeholder properly", function() {
        assert.strictEqual(`${undefined}`, t`${undefined}`);
      });

      it("should handle null placeholder properly", function() {
        assert.strictEqual(`${null}`, t`${null}`);
      });

      it("should handle object placeholder properly", function() {
        const obj = {x: 56};
        assert.strictEqual(`${obj}`, t`${obj}`);
      });
    });
  ...
  a new line`,
      ...
      it("should remove CRLF newlines", function() {
        assert.strictEqual(t`this has \r\n a new line`, "this hasa new line");
      });
  ...
  this has a leading new line`,
      ...
      it("should remove trailing newlines without leaving trailing spaces", function() {
        assert.strictEqual(
          t`this has a trailing new line
  `,
          "this has a trailing new line"
        );
      });
